...
initial begin
    for (k = 0; k < DEPTH; k = k + 1)
        ref_mem[k] = 128'h00000000000000000000000000000000;

    ref_mem[13'h0000] = 128'hABCDEF01234567890123456789ABCDEF;
    ref_mem[13'h0001] = 128'h0123456789ABCDEFFEDCBA9876543210;
    ref_mem[13'h0002] = 128'hFEDCBA98765432101122334455667788;
    ref_mem[13'h0003] = 128'h11223344556677888877665544332211;
    ref_mem[13'h0004] = 128'h8877665544332211AABBCCDDEEFF0011;
    ref_mem[13'h0005] = 128'hAABBCCDDEEFF00111100FFEEDDCCBBAA;
    ref_mem[13'h0006] = 128'h1100FFEEDDCCBBAADEADBEEFCAFEBABE;
    ref_mem[13'h0007] = 128'hDEADBEEFCAFEBABECAFEBABEDEADBEEF;
    ref_mem[13'h0008] = 128'hCAFEBABEDEADBEEF0F0F0F0F0F0F0F0F;
    ref_mem[13'h0009] = 128'h0F0F0F0F0F0F0F0FF0F0F0F0F0F0F0F0;
    ref_mem[13'h000A] = 128'hF0F0F0F0F0F0F0F0AAAAAAAAAAAAAAAA;
    ref_mem[13'h000B] = 128'hAAAAAAAAAAAAAAAA5555555555555555;
    ref_mem[13'h000C] = 128'h5555555555555555FFFFFFFFFFFFFFFF;
    ref_mem[13'h000D] = 128'hFFFFFFFFFFFFFFFF0000000000000001;
    ref_mem[13'h000E] = 128'h00000000000000018000000000000000;
    ref_mem[13'h000F] = 128'h80000000000000000000000000000000;
end
...
begin
    ...
    while (!done && timeout < (expected_total + 1000)) begin
        ...
        if (out_valid) begin
            ...
            $fwrite(file_out,
                    "ciclo=%0d rep=%0d addr=%04h data=%032h esperado=%032h",
                    cycle_counter,
                    expected_repeat,
                    expected_addr[12:0],
                    out_data,
            ...
            if (out_data !== expected) begin
                $display("[TC%0d] FAIL ciclo=%0d rep=%0d addr=%04h out=%032h esperado=%032h",
                         tc_num,
                         cycle_counter,
                         expected_repeat,
                         expected_addr[12:0],
                ...
                if (received_total < 20) begin
                    $display("[TC%0d] PASS ciclo=%0d rep=%0d addr=%04h data=%032h",
                             tc_num,
                             cycle_counter,
                             expected_repeat,
                             expected_addr[12:0],
...
begin
    ...
    while (received < 16) begin
        ...
        if (out_valid) begin
            ...
            $fwrite(file_out,
                    "addr=%04h data=%032h esperado=%032h",
                    received[12:0],
                    out_data,
                    expected);
...
initial begin
    ...
    $display("============================================================");
    $display(" TESTBENCH - INPUT_BUFFER PREPARE / GO - 128 BITS");
    $display("============================================================");

    $fwrite(file_out, "============================================================\n");
    $fwrite(file_out, "RELATORIO - INPUT_BUFFER PREPARE / GO - 128 BITS\n");
    $fwrite(file_out, "MIF: DEPTH=8192 WIDTH=128 ENDERECOS 0000..1FFF\n");
    $fwrite(file_out, "============================================================\n");
